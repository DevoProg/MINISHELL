...
void first_cmd(t_data *minis, t_board *cmd, char **envp, int fd[2][2])
{
    cmd->res_fork = fork();
    if (cmd->res_fork < 0) {
        return ;
...
void last_cmd(t_data *minis, t_board *cmd, char **envp, int fd[2][2])
{
    cmd->res_fork = fork();
    if (cmd->res_fork < 0) {
        return ;
...
//     // fd[1] = malloc(sizeof(int) * 2);
    ...
    last_cmd(minis, &minis->cmd[2], envp, fd);
    close(fd[0][0]);
    close(fd[0][1]);
    close(fd[1][0]);
    close(fd[1][1]);
